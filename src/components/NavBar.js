...
export default function NavBar() {
   ...
   function handleLinkClick(data){
      ...
      navigate('/Dashboard', { state: { lngLat:lngLatArray, ejamaat: data.ejamaat }})
   }

   async function getTotalBohraCount() {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/bohra-count`);
   ...
   function onBohraEvent(data){
      ...
      toast.info( 
         <>
          ...
          <br />
          <button type="button" className="btn btn-primary btn-sm rounded" onClick={() => handleLinkClick(data)}>Connect</button>
         </>
         , {
   ...
   return (
      <div className="bg-light nav-bar-container">
         ...
         <div className="anchor-container">
            ...
            <span className={isFlashing ? flashingStyle : 'btn btn-dark'}>{bohraCount}</span>
         </div>
         <ToastContainer />
